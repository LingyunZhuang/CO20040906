...
const states = {
  ...
  start: () => {
    ...
    if (state.ifPractice!==1) {
      console.log(speeds[state.speedStart-1],'-----------')
      console.log(speeds,'-----------','state.speedStart',state.speedStart)
      store.commit('updateRecords',[...state.records,{
        name: localStorage.getItem('name'),
        beginTime: getDate(),
        endTime: '',
        speed: speeds[state.speedStart-1],
        rows: 0,
        curr: store.state.curr
      }])
    }
  ...
  nextAround: (matrix, stopDownTrigger) => {
    ...
    const state = store.state
    // console.log(speeds,'=========',speeds[store.state.speedStart-1])
    if(state.ifPractice === 1) {
    ...
    if (isOver(matrix)) {
      if (state.ifPractice !== 1) {
        let record = state.records[state.records.length-1]
        record.endTime = getDate()
        record.rows = state.clearLines
        store.commit('updateRecords',[...state.records])
      }
      if (music.gameover) {
        music.gameover()
  ...
  overStart: () => {
    ...
    if (store.state.ifPractice===1) {
      speedRunNow = Math.max(2, state.speedRun - 1)
    } else {
      speedRunNow = state.speedRun
    }
    store.commit('speedRun', speedRunNow)
